...
def _find_active_duplicate(db: Session, user_id: uuid.UUID, file_hash: str) -> Document | None:
	return (
		db.query(Document)
		.filter(
...
def precheck_document(
	...
	duplicate = _find_active_duplicate(db, current_user.id, payload.file_hash)
	if not duplicate:
		return PrecheckDocumentResponse(duplicate=False, existing_document=None)

	summary = DuplicateDocumentSummary(
		id=duplicate.id,
		filename=duplicate.filename,
		status=duplicate.status,
		created_at=duplicate.created_at,
	)
	return PrecheckDocumentResponse(duplicate=True, existing_document=summary)
...
async def upload_document_file(
	...
	duplicate = _find_active_duplicate(db, current_user.id, file_hash)
	if duplicate:
		summary = {
			"id": str(duplicate.id),
			"filename": duplicate.filename,
		...
		raise HTTPException(
			...
			detail={"message": "Duplicate file detected", "existing_document": summary},
		)
...
def confirm_upload(
	...
	token_hash = str(upload_payload.get("file_hash", "")).lower()
	...
	duplicate = _find_active_duplicate(db, current_user.id, token_hash)
	if duplicate:
		summary = {
			"id": str(duplicate.id),
			"filename": duplicate.filename,
...
def get_document(
	...
	document = _get_owned_document(db, current_user, document_id, include_deleted=True)
